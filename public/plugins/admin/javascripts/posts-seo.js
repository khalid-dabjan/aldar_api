 ...
 * @returns {string}
 */
function amtFocusKwTest(str, p) {
    str = amtClean(str);
    str = str.toLocaleLowerCase();
    str = amtRemoveLowerCaseDiacritics(str);
 ...
 * @returns {string}
 */
function amtRemoveLowerCaseDiacritics(str) {
 ...
 */
function amtTestFocusKw() {
    // Retrieve focus keyword and trim
    var focuskw = jQuery.trim(jQuery('#focus_keyword').val());

    focuskw = amtEscapeFocusKw(focuskw).toLowerCase();
    ...
    if (focuskw !== '') {
        var html = '<p><strong>' + focus_keyword_usage + '</strong><br>' + focus_keyword + '</p>';
        html += '<ul>';
        if (jQuery('#post_title').length) {
            html += '<li>' + page_title + amtFocusKwTest(jQuery('#post_title').val(), p) + '</li>';
        }
        html += '<li>' + seo_title + amtFocusKwTest(jQuery('#wpseosnippet_title').text(), p) + '</li>';
        html += '<li>' + page_url + amtFocusKwTest(url, p2) + '</li>';

        var content = "";
        if (mongo_id) {
            content = $("#mongo-content").html();
        } else {
            content = CKEDITOR.instances['pagecontent'].getData();
        }

        if (content.length) {
            html += '<li>' + post_content + amtFocusKwTest(content, p) + '</li>';
        }
        html += '<li>' + seo_description + amtFocusKwTest(metadesc, p) + '</li>';
        html += '</ul>';
 ...
 *
 * This is done through a callback as it _could_ be that `amtReplaceVariables` has to do an AJAX request.
 *
 ...
 */
function amtReplaceVariables(str, callback) {
...
//    str = str.replace(/%%currentyear%%/g, wpseoMetaboxL10n.currentyear);
    ...
    str = str.replace(/%%focuskw%%/g, jQuery('#focus_keyword').val().replace(/(<([^>]+)>)/ig, ''));
    // excerpt
    var excerpt = '';
    var contentT = '';
    if (mongo_id) {
        contentT = $("#mongo-content").html();
    } else {
        contentT = CKEDITOR.instances['pagecontent'].getData();
    }
    if (jQuery('#post_excerpt').length) {
        excerpt = amtClean(jQuery('#post_excerpt').val().replace(/(<([^>]+)>)/ig, ''));
        str = str.replace(/%%excerpt_only%%/g, excerpt);
    }
    if ('' === excerpt && contentT.length) {
        excerpt = contentT.replace(/(<([^>]+)>)/ig, '').substring(0, wpseoMetaboxL10n.wpseo_meta_desc_length - 1);
    }
...
//    var esc_sep = amtEscapeFocusKw(wpseoMetaboxL10n.sep);
...
//                amtAjaxReplaceVariables(replaceableVar, callback);
//            }
//        }
//    }
    callback(str);
 ...
 */
//function amtAjaxReplaceVariables(replaceableVar, callback) {
//    jQuery.post(ajaxurl, {
...
//        }
//
//        amtReplaceVariables(replaceableVar, callback);
//    }
//    );
//}
 ...
 */
function amtUpdateTitle(force) {
    var title = '';
    var titleElm = jQuery('#meta_title');
    var titleLengthError = jQuery('#title-length-warning');
    ...
    if (titleElm.val()) {
        ...
        title = $("#post_title").val() + sitename;
        title = divHtml.html(title).text();
    }
    if (title === '') {
        snippetTitle.html('');
        titleLengthError.trigger('mouseout');
        return;
    }

    title = amtClean(title);
    ...
    title = amtReplaceVariables(title, function (title) {
        title = amtSanitizeTitle(title);
        ...
        if (titleElement !== null) {
            if (titleElement.scrollWidth > titleElement.clientWidth) {
                titleLengthError.trigger('mouseenter');
            }
            else {
                titleLengthError.trigger('mouseout');
            }
 ...
 * @returns {string}
 */
function amtSanitizeTitle(title) {
    title = amtClean(title);

    // and now the snippet preview title
    title = amtBoldKeywords(title, false);
 ...
 */
function amtUpdateDesc() {
    var desc = jQuery.trim(amtClean(jQuery('#meta_description').val()));
    var divHtml = jQuery('<div />');
    var snippet = jQuery('#wpseosnippet');

    if (desc !== '') {
        desc = amtReplaceVariables(desc, function (desc) {
            desc = divHtml.text(desc).html();
            desc = amtClean(desc);

            var len = 156 - desc.length;
        ...
        if ($('#post_excerpt').val() !== null) {
            desc = $('#post_excerpt').val();
            desc = amtClean(desc);
        }
        ...
        if (content !== null && desc.length === 0) {
            desc = content;

            desc = amtClean(desc);
        }

        var focuskw = amtEscapeFocusKw(jQuery.trim(jQuery('#focus_keyword').val()));
        if (focuskw !== '') {
            var descsearch = new RegExp(focuskw, 'gim');
            if (desc.search(descsearch) !== -1 && desc.length > 156) {
                desc = desc.substr(desc.search(descsearch), 156);
            }
            else {
 ...
 * @returns {string}
 */
function amtSanitizeDesc(desc) {
    desc = amtTrimDesc(desc);
    desc = amtBoldKeywords(desc, false);
 ...
 * @returns {string}
 */
function amtTrimDesc(desc) {
    if (desc.length > 156) {
        var space;
        if (desc.length > 156) {
            space = desc.lastIndexOf(' ', (156 - 3));
        }
        else {
            space = 156;
        }
 ...
 */
function amtUpdateURL() {
    var url;
    if (jQuery('#editable-post-name-full').length) {
        var name = jQuery('#editable-post-name-full').text();
        url = postURL.replace('http://', '');
    }
    url = amtBoldKeywords(url, true);
 ...
 * @returns {string}
 */
function amtBoldKeywords(str, url) {
    var focuskw = amtEscapeFocusKw(jQuery.trim(jQuery('#focus_keyword').val()));
    var keywords;
    ...
    for (var i = 0; i < keywords.length; i++) {
        ...
        if (str !== undefined) {
            str = str.replace(kwregex, '$1<strong id="wpseosnippet_slug">$2</strong>$3');
        }
    }
    return str;
}
...
jQuery(document).ready(function () {
    ...
    jQuery('#meta_title').keyup(function () {
        amtUpdateTitle();
    });
...
//                    tinyMCE.get('content').on('blur', amtUpdateDesc);
//                }
//
//                if (typeof tinyMCE !== 'undefined' && tinyMCE.get('excerpt') !== null) {
//                    tinyMCE.get('excerpt').on('blur', amtUpdateDesc);
//                }
//            },
//            500
//            );

    jQuery(document).on('change', '#focus_keyword', function () {
        var focuskwhelpElm = jQuery('#focuskwhelp');
        if (jQuery('#focus_keyword').val().search(',') !== -1) {
            focuskwhelpElm.trigger('mouseenter');
            focuskwHelpTriggered = true;
        }
        else if (focuskwHelpTriggered) {
            focuskwhelpElm.trigger('mouseout');
